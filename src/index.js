...
//  AND id_route = (SELECT MAX(id_route) FROM route_p2)       AND id_route = (SELECT id_route FROM route_p2 WHERE id_user = ? ORDER BY id_route DESC LIMIT 1)
                    connection.query(' UPDATE route_p2 SET time_beg = ADDTIME (NOW(), "03:00:00"), time_end = ADDTIME (NOW(), "03:40:00") WHERE id_user = ? AND id_route = (SELECT MAX(id_route) FROM route_p2 WHERE id_user = ? ) ',
